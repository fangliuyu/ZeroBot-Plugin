...
func init() {
	en := control.Register("ygocdb", &ctrl.Options[*zero.Ctx]{
		...
		Help: "百鸽卡查(ygocdb.com)的API卡查\n" +
			"/ydb [xxx]\n" +
			"-[xxx]为搜索内容\n",
	})

	en.OnRegex(`^(.{1})ydb\s?(.+)$`, zero.OnlyGroup).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		ctxtext := ctx.State["regex_matched"].([]string)[2]
		// 获取网络数据并进行json解析
		searchName := api + url.QueryEscape(strings.ReplaceAll(ctxtext, " ", "+")) // 条件以“+”连接
		data, err := web.GetData(searchName)
		if err != nil {
