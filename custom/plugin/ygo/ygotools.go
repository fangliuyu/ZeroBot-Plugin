...
func init() {
	ygotools := control.Register("ygo", "ygotools", &ctrl.Options[*zero.Ctx]{
	...
	// 村规
	ygotools.OnFullMatchGroup([]string{"/村规", ".村规", "村规", "群规", "暗黑决斗"}, func(ctx *zero.Ctx) bool {
		return ctx.Event.GroupID == 979031435
	}).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		ctx.SendChain(message.Text(ygorule))
	})
	// 房间
	ygotools.OnRegex(`^[(.|。|\/|\\|老|开)](房间|单打|双打|比赛)(\s.*)?`).SetBlock(true).Handle(func(ctx *zero.Ctx) {
