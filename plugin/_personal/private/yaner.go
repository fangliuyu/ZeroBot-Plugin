...
func init() { // 插件主体
	...
	// 撤回最后的发言
	engine.OnRegex(`^\[CQ:reply,id=(.*)].*`, zero.AdminPermission, zero.KeywordRule("多嘴")).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			// 获取消息id
	...
	engine.OnKeyword("测试语音", zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			ctx.SendChain(message.Record(res + "az.wav"))
		})
	...
	engine.OnKeyword("help", zero.OnlyToMe).SetBlock(true).Limit(ctxext.LimitByGroup).Handle(func(ctx *zero.Ctx) {
		process.SleepAbout1sTo2s()
		if rand.Intn(2) == 0 {
